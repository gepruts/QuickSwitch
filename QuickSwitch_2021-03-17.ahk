...
;			WinGetClass, _parentClass, ahk_id %_parentHandle%
		...
		;	Get handle of grandparent control
			_grandparentHandle := DllCall("GetParent", "Ptr", _parentHandle)

;		;	Get class of grandparent control
;			WinGetClass, _grandparentClass, ahk_id %_grandparentHandle%

		;	Get handle of greatgrandparent control
			_greatgrandparentHandle := DllCall("GetParent", "Ptr", _grandparentHandle)

		;	Get class of greatgrandparent control
			WinGetClass, _greatgrandparentClass, ahk_id %_greatgrandparentHandle%

;MsgBox %A_LoopField% parent = %_parentClass% grandparent = %_grandparentClass% greatgrandparent = %_greatgrandparentClass%
; right order:
;EditX parent = ComboBox grandparent = ComboBoxEx32 greatgrandparent = msctls_progress32

			If ( _greatgrandparentClass = "msctls_progress32" )
				_addressEdit := A_LoopField

;			_parentHandle			:= ""
;			_parentClass			:= ""
;			_grandparentHandle	:= ""
;			_grandparentClass		:= ""
;			_greatgrandparentHandle	:= ""
;			_greatgrandparentClass		:= ""

		}
	}


	If ( _useToolbar AND _EnterToolbar AND _addressEdit )
	{
;MsgBox Controls OK _thisFOLDER = %_thisFOLDER%
	;	Click address toolbar until Edit 2 visible. Fill Edit 2 with new folder	
		Loop, 50
		{
			...
			Else			; Edit2/_addressEdit visible?
			{
				ControlGet, _Edit2Visible, Visible, , %_addressEdit%, ahk_id %_thisID%
				Sleep, 10

				If (_Edit2Visible)
				{
					ControlSetText, %_addressEdit%, %_thisFOLDER%, ahk_id %_thisID%
					Sleep, 10
					ControlGetText, _Edit2, %_addressEdit%, ahk_id %_thisID%
