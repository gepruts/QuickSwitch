...
- change menu folder order from fixed (TC > XY > Explorer) to most recent first.
...
- TEST
? Add menu: debugging > z-order?
? Add menu: debugging > list all controls (+text)?
? Add "exception-detector" (Z=3)
...
If ($Debug)
MsgBox DEBUG MAIN dialogtype = %$DialogType%
...
{
If ($Debug)
MsgBox DEBUG Start SmellsLikeAFileDialog
...
;MsgBox 1
	...
	Sleep, 50
	WinGet, _thisEXE, ProcessName, ahk_id %_thisID%

Global $DialogType
If ($Debug)
MsgBox DEBUG FeedDialogType %$DialogType%
...
If ($Debug)
MsgBox DEBUG FeedDialog GENERAL activated
...
; ================= Regular File Dialog ====================================
	...
	} else If ( $DialogType = "SYSLISTVIEW" ) {

If ($Debug)
MsgBox DEBUG FeedDialog SYSLISTVIEW activated
...
If ($Debug)
MsgBox DEBUG NOT A valid dailog activated
...
If ($Debug)
MsgBox Folderchoice dialogtype = %$DialogType%
